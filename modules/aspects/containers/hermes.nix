...
{ den, ... }:
{
  den.aspects.hermes = {
    ...
    nixos = { lib, host, ... }: {
      ...
      systemd.services.podman-hermes = lib.dot.mkContainerDeps "hermes" // {
        after = [ "podman-network-${lib.dot.containerNetwork}.service" "podman-camofox-browser.service" ];
        requires = [ "podman-camofox-browser.service" ];
      };
    };
  };
}
